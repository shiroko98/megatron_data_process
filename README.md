...
- [Mobius-r6-chat-CHNtuned-12b-16k-v6_5-0609](Mobius-r6-chat-CHNtuned-12b-16k-v6_5-0609/)
- [yi](yi/)

其中还有一种需要特别说明的情况：

- 像 `Mobius-r6-chat-CHNtuned-12b-16k-v6_5-0609` 这样的目录，虽然是用 RWKV 词表文件，但它走的是 Hugging Face 自定义 tokenizer 代码，不是 `pyrwkv_tokenizer`
- 这类自定义 HF tokenizer 在当前实现里会直接按 Python 语义解析词表，因此既能读取老式混合写法
  - 例如：`'hello'`、`b'\x80'`
- 也能读取全 `b'...'` 写法
  - 例如：`b'hello'`、`b'<|im_start|>'`
- 但这不代表所有 HF tokenizer 都天然支持这种 RWKV 词表格式；这是该模型自带自定义 tokenizer 实现提供的兼容性
- 相对地，`RWKVTokenizer` 走的 `pyrwkv_tokenizer` 后端主要兼容老的规范词表格式，所以两条路径不要混用理解
